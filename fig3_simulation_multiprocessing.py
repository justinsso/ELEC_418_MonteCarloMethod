...
def _run_one(args):
    zf_lt, N_photons = args
    return (zf_lt, run_simulation_fig3(zf_lt * L_T_PRIME, N_photons))


if __name__ == "__main__":
    jobs = [(zf_lt, N_PHOTONS) for zf_lt in ZF_OVER_LT]
    with multiprocessing.Pool() as pool:
        results_list = pool.map(_run_one, jobs)
